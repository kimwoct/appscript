...
function readRow2AllSheets() {
  ...
  sheets.forEach(function (sheet) {
    ...
    results[sheet.getName()] = row2;
    Logger.log('Sheet: ' + sheet.getName() + ' | Row 2: ' + row2);
  });
  Logger.log(results); // This will log the entire results object at the end
}

/*
Here is a complete output table with all names and their extracted **teacher codes** (the text inside parentheses):
...
function copyLessonInfo() {
  ...
  for (var i = 0; i < teacherCode.length; i++) {
    ...
    if (sourceSheet && destSheet) {
      var sourceRange = sourceSheet.getRange("B5:G18");
      var values = sourceRange.getValues();
      destSheet.getRange("D7:I20").setValues(values);
...
function updateLessonInfo() {
  ...
  for (var i = 0; i < TEACHERCODE.length; i++) {
    var destSheet = ss.getSheetByName(TEACHERCODE[i]);
    if (destSheet) {
      var destRange = destSheet.getRange("D7:I20");
...
var OLD_TEACHER_LIST = ['姚寶芬', '林鐃程', '莊鳳儀', '黃雋之', '李美姍', '溫婉慈', '楊曉怡', '陳藹儀', '鄧紫晴', '張頴儀', '王麗蓮', '劉雅恩', '黃俊基', '陳晉芝', '尹兆恒', '王翠儀', '王彥敏', '容淑儀', '李志豪', '李海言', '李珈琳', '林善嬴', '梁美詩', '楊麗芳', '潘國光', '潘濬仁', '羅宇琪', '葉兆彤', '葉曉文', '譚曉嵐', '陳振秋', '顏詠詩', '馮碧珊', '黃慧萍', '黃旭俊', 'Allyson Holde'];
...
//Step 7: Find the new teacher list teacher code
function getNewTeacherCode() {
  var NEW_TEACHER_LIST = ['姚寶芬','林鐃程','KARY','黃雋之','李美姍','SENTA','楊曉怡','陳藹儀','鄧紫晴','梁思晴','王麗蓮','劉雅恩','黃俊基','陳晉芝','尹兆恒','王翠儀','王彥敏','容淑儀','朱明霞','李志豪','李海言','李珈琳','林善嬴','梁美詩','楊麗芳','潘國光','潘濬仁','羅宇琪','葉兆彤','葉曉文','譚曉嵐','陳振秋','顏詏詩','馮碧珊','鄧海玲','黃旭俊','ALLYSON HOLDER'];
